...
function generateDeclaracaoCienciaDeveresResponsabilidadesPDF() {
    ...
    const dirigente = document.getElementById('dirigente').value;
    const rgDirigente = document.getElementById('rg').value;
    const orgaoDirigente = document.getElementById('orgao').value;
    const cpfDirigente = document.getElementById('cpf').value;
    const entidade = document.getElementById('entidade').value;
    const cnpjEntidade = document.getElementById('cnpj').value;
...
function generateAllPDFs() {
    ...
    setTimeout(generateDeclaracaoNaoUtilizacaoRecursosPDF, 1000);
    setTimeout(generateDeclaracaoLeiPDF, 2000);
    setTimeout(generateDeclaracaoAusenciaRecursosPDF, 3000);
    setTimeout(generateDeclaracaoCumprimentoArt90PDF, 4000);
    setTimeout(generateDeclaracaoNaoContratacaoRecursosParceriaPDF, 5000);
    setTimeout(generateDeclaracaoArt299AutonomiaFinanceiraPDF, 6000);
    setTimeout(generateDeclaracaoNaoOcorrenciaImpedimentosPDF, 7000);
    setTimeout(generateDeclaracaoNaoRecebeRecursosPDF, 8000);
    setTimeout(generateDeclaracaoComprovacaoExistenciaPDF, 9000);
    setTimeout(generateDeclaracaoCompromissoPDF, 10000);
    setTimeout(generateDeclaracaoComprovacaoExistenciaPDF, 11000);
    setTimeout(generateDeclaracaoCustosPDF, 12000);
    setTimeout(generateTermoCompromissoCoordenadorPDF, 13000);
    setTimeout(generateDeclaracaoAdimplenciaPDF, 14000);
    setTimeout(generateDeclaracaoCienciaDeveresResponsabilidadesPDF, 15000);
}

// Evento de clique no botão para gerar todos os PDFs
